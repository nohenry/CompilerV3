...
use ts_lexer::OperatorKind;

use super::module::Module;
use ts_symbol::{Symbol, SymbolFlags, SymbolValue, Type, Value};
                        ...
                        if let Some(current) = current {
                            ...
                            pram_iter.for_each(|(name, bounds)| {
                                let found = ty.parameters.iter().position(|f| {
                                    if let ts_lexer::ast::Type::NamedType(t) = &f.symbol_type {
                                        if &t.as_string() == name {
                                            return true;
                                        }
                                    }
                    ...
                    // Generate type arguments
                    {
                        ...
                        if let Some(current) = current {
                            ...
                            let p: Option<()> = pram_iter.map(|(name, bounds)| {
                                fn find(f: &ts_lexer::ast::Type, name: &String) -> bool {
                                    match f {
                                        ts_lexer::ast::Type::NamedType(t) => {
                                            if &t.as_string() == name {
                                                return true;
                                            }
                                        },
                                        ts_lexer::ast::Type::GenericType(t) => {
                                    ...
                                    if let Some(sym) = current.children.get_mut(name) {
                                        match &mut sym.value {
                                            SymbolValue::Generic(gty, _) => {
                                                if let ts_lexer::ast::Type::GenericType(gen) = &pt.symbol_type {
