...
use ts_errors::{check, CodeGenError};
use ts_llvm::IRBuilder;
...
use ts_symbol::{GenericType, Symbol, SymbolFlags, SymbolValue, Type, Value};
        ...
        match node {
            ...
            ParseNode::ActionDecleration(ActionDecleration {
                ...
                if let Some(generic) = generic {
                    let ts_lexer::ast::Type::GenericType(ts_lexer::ast::GenericType {base_type: box ts_lexer::ast::Type::NamedType(tok), ..}) = template_type else {
                        // TODO: errors
                        return Value::Empty;
                    };
                        ...
                        for fun in st {
                            if let ParseNode::FunctionDecleration(func) = fun {
                                ...
                                let prms: Vec<TypeSymbol> = match i.next() {
                                    Some(f) => {
                                        if let ts_lexer::ast::Type::ConstSelf = &f.symbol_type {
                                            [TypeSymbol {
                                                symbol: Token {
                                                    token_type: TokenKind::Keyword(Keyword {
                                                        keyword: ts_lexer::KeywordKind::Const,
                                            ...
                                            .chain(i)
                                            .collect()
                                        } else if let ts_lexer::ast::Type::SELF = &f.symbol_type {
                                            [TypeSymbol {
