use ts_errors::check;
use ts_symbol::{Symbol, SymbolValue};
...
use ts_lexer::ast::{ArrayType, FunctionType, GenericType, ReferenceType, Type};

use super::module::Module;

impl Module {
    pub(super) fn gen_type(&self, _type: &Type) -> ts_symbol::Type {
        match _type {
            Type::Int(s, _) => unsafe {
                ts_symbol::Type::Integer {
            ...
            Type::Float(s, _) => unsafe {
                match *s {
                    32 => ts_symbol::Type::Float {
                        llvm_type: LLVMFloatType(),
                    },
                    64 => ts_symbol::Type::Float {
                        llvm_type: LLVMDoubleType(),
                    },
                    _ => ts_symbol::Type::Empty,
                }
            },
            ...
            Type::GenericType(GenericType {
                ...
                let ts_symbol::Type::TemplateTemplate { path, fields, ty_params, existing, specialization }= self.gen_type(base_type) else {
                    return ts_symbol::Type::Empty
                };
                let parameters: Vec<ts_symbol::Type> =
                    ...
                    if let Some(ty) = existing_impl {
                        ...
                        let (template, name) = {
                            ...
                            let opvars = if let Some(special) = specialization {
                                ...
                                if let Some(Symbol {
                                    value:
                                        SymbolValue::Template(ts_symbol::Type::TemplateTemplate {
                                            fields,
                                            ..
                                        }),
                                    ..
